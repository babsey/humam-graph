...
function addArc(ref, data, level = 1, r = 24) {
  ...
  ref
    ...
    .attr("d", (d) => arc(d))
    .attr("fill", (d) =>
      filterLinksBy.value.length === 0 || filterLinksBy.value.includes(d.id) ? color(d.id) : "var(--color-text)"
    )
...
const update = () => {
  ...
  function overed(_, d) {
    // links
    svg.selectAll(".link").selectAll("path").attr("stroke", "var(--color-text)").style("opacity", 0.05);
